...
# Exercice 15 : Votre première clé de signature
...
```bash
keytool -genkeypair -v -keystore MaSalle.keystore -alias cle_signature_masalle -keyalg RSA -keysize 2048 -validity 10000
```
...
* **Fichier de secours local :** Une copie physique locale hors-ligne a été placée dans un répertoire utilisateur privé sur la machine hôte (`C:\Users\erwan\.security\`), un dossier qui est strictement exclu de tout suivi Git via le fichier `.gitignore`.
